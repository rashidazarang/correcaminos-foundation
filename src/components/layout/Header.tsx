    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-6 md:px-12 lg:px-20 h-20 md:h-24">
          <Link
            to="/"
            className={`font-display text-xl md:text-2xl font-medium tracking-wide transition-colors duration-300 ${
          ...
          <button
            ...
            className={`relative z-50 w-10 h-10 flex flex-col items-center justify-center gap-1.5 transition-colors ${
              hasScrolled || isMenuOpen ? "text-blanco-sal" : "text-negro-asfalto"
            }`}
            ...
            >
              {navLinks.map((link) => (
                ...
                >
                  <Link
                    to={link.path}
                    className={`font-display text-3xl md:text-5xl lg:text-6xl text-blanco-sal relative group ${
                      location.pathname === link.path ? "text-primary" : ""
                    }`}
                  >
                    {link.name}
                    <span className="absolute bottom-0 left-0 w-full h-[3px] bg-primary origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300" />
                  </Link>
                </motion.li>
              ))}
            </motion.ul>
